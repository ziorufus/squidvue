...
from app.models import Answer, Participant, Quiz, User, UserRole
...
def ranking(code: str, request: Request, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    quiz = db.scalar(select(Quiz).where(Quiz.code == code.upper()))
    if not quiz:
        raise HTTPException(status_code=404, detail='Quiz not found')
    runtime = request.app.state.runtime
    include_email = user.role == UserRole.ADMIN
    return {
        'quiz': runtime.ranking_for_quiz(db, quiz.id, include_email=include_email),
        'global': runtime.global_ranking(db, include_email=include_email),
    }


@router.get('/ranking/global')
def global_ranking(request: Request, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    runtime = request.app.state.runtime
    return {
        'global': runtime.global_ranking(db, include_email=user.role == UserRole.ADMIN),
    }
