...
class QuizRuntime:
    ...
    def ranking_for_quiz(self, db: Session, quiz_id: int, include_email: bool = False) -> list[dict]:
        rows = db.execute(
            select(User.emoji, User.email, func.sum(Answer.score).label('score'))
            ...
            .group_by(User.emoji, User.email)
        ...
        return [
            {
                ...
                **({'email': row[1]} if include_email else {}),
            }
            for row in rows
        ]

    def global_ranking(self, db: Session, include_email: bool = False) -> list[dict]:
        rows = db.execute(
            select(User.emoji, User.email, func.sum(Answer.score).label('score'))
            .select_from(Participant)
            .join(User, User.id == Participant.user_id)
